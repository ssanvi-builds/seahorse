"""LLM CI gate integration tests — real Ollama, weakest model.
...
quality of a strong model. A dev who opts into ``json_schema`` without the
plain-prompt fallback makes THIS gate red, so the coupling is caught in CI, not
in production.
...
# The weakest family member the gate is contractually run against.
...
# a bare date from the content as ``valid_at``, which the validator rejects
...
def _assert_gate_model_provenance(model_used: str) -> None:
    """Provenance must carry the effective model WITH its tag."""
...
class TestExtractFlowReal:
    def test_weak_model_extracts_subject_with_plain_prompt(self, monkeypatch) -> None:
        ...
        # native structured output: the gate is RED if extraction only works
...
class TestRepairFlowReal:
    def test_repair_loop_runs_against_real_model_and_degrades_honestly(
        ...
        monkeypatch.setattr(backend, "_complete_one", _spy)
        # repair_budget=1 → exactly one repair re-prompt.
        res = backend.extract(
        ...
        if res.degraded_to_skip:
            # Honest degrade: the weak model could not fix its output within the
            # repair budget — never a crash, never garbage. The provenance
            # hashes the FIRST prompt (the backend's degraded path).
            assert res.data == {}
            ...
            # prompt that produced the valid output.
            assert res.data["subject"].strip()
...
class TestRunLlmPathEndToEnd:
    def test_write_path_produces_llm_provenance_on_weak_model(self) -> None:
        ...
        assert call.subject, "weak model extraction must reach engine.remember"
