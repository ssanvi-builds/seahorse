...
class TestFullTwoStage:
    def test_all_four_sources_contribute(
        ...
        episode_repo.add(e1)
        bfs_repo.rows = [_row("eB", created_at=NOW), _row("eC", created_at=NOW)]
        result = recall(
            "q",
            pit=None,
            embedder=embedder,
        ...
        # e1 is in vector + bm25 + chain (active-now anchor). NOT in bfs (eB/eC).
        assert set(by_id["e1"].sources) == {"bm25", "chain", "vector"}
        # e2 only vector; e3 only bm25; eB/eC only bfs.
        assert by_id["e2"].sources == ("vector",)
        assert by_id["e3"].sources == ("bm25",)
        assert by_id["eB"].sources == ("bfs",)

    def test_anchor_drives_both_chain_and_bfs(
        self, embedder, vector_repo, fts_repo, episode_repo, bfs_repo, fixed_clock
    ):
        ...
        # The stage-1 top-1 (e1) is the anchor for BOTH chain and BFS.
        assert episode_repo.chain_calls == ["e1"]
        assert bfs_repo.calls[0]["ep_id"] == "e1"
