"""MemoryFacade — the canonical Python API for the memory-native primitives.

The facade is **clean delegation**: routing + boundary shape-validation +
primitive logging. It does NOT host domain logic. It does not fuse (hybrid
retrieval), project (progressive disclosure), extract (the write path / LLM
backend), open transactions (``repo.atomic()`` lives in the engine), construct
``IndexRow`` (progressive disclosure owns that), derive ``subject``/``fact_id``
(the engine), or build ``supersedes`` (the engine). The ``source_type → skip``
guard lives in the write path's ``decide_path`` — the facade does not replicate
it.

The four memory-native primitives + three progressive-disclosure read levels:
- ``remember``  → the write path's ``WritePath.ingest`` (single write-path).
- ``recall``    → the current-state listing via ``materialize_index``
                  (synthetic ``FusedCandidate`` score=0.0; no ranking, no PIT).
- ``recall_timeline`` / ``recall_full`` → ``materialize_timeline`` / ``materialize_full``.
- ``improve`` / ``forget`` → ``engine.improve`` / ``engine.forget`` directly.
...
Boundary failures raise ``SeahorseError`` (stable ``code``). Later-release
primitives (``expire`` / ``revalidate``) raise ``E_NOT_IN_MVP_0_1`` at the
border.
"""
...
class _RetrieverLike(Protocol):
    """Recall-policy extension point: produces ranked ``FusedCandidate`` for progressive disclosure.

    The current-release impl is ``VigenteListingRetriever`` (current-state
    listing, no ranking/PIT). A later release swaps in an adapter over
    ``seahorse.retrieval.recall`` (kNN+BM25+RRF) at the composition root — a
    single-point change. The facade owns boundary validation (empty query, PIT
    refusal) + the shaper call; the retriever owns listing/filter/truncate +
    ranking.

    PIT capability is a DATA attribute (not a method) — the facade reads
    ``getattr(retriever, "supports_pit", False)`` before delegating. A retriever
    that does not declare it (e.g. the listing regime) causes the facade to
    refuse a caller pit before any read; the later-release ``HybridRetriever``
    declares it True and receives the pit verbatim. ``@runtime_checkable`` does
    NOT check data attributes, which is why the facade uses ``getattr`` with a
    False default.
    """
...
class MemoryFacade:
    """The canonical Python API for the memory-native primitives.

    Construct with the engine, write-path, and shaper it delegates to. ``clock``
    makes ``now`` injectable (reproducibility); ``primitive_log`` lets tests
    assert the primitive was emitted.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Composition-root ``embedder`` slot (a later-release seam). Current
        # recall is the current-state listing and never embeds, so the slot is
        # inert; the default stub raises E_NOT_IN_MVP_0 if a non-skip path
    ...
    def remember(
        ...
        *,
        ...
        """Remember a fact (delegates to the write path's ``WritePath.ingest``).

        Boundary shape-validation only: body non-empty, ``by.source_type``
        present (caller authority), ``tags`` empty in the current release,
        resolved ``extraction_mode`` ∈ {skip, llm}. Domain invariants (valid_at
        guard, cognitive_type, collision, idempotency) are the engine's / write
        path's authority.
        """
    ...
    def _validate_remember_payload(self, payload: RememberPayload) -> None:
        ...
        if payload.tags:
            raise SeahorseError(
                ...
                detail="tags are not supported in this release's remember",
            )
            ...
            if extraction_mode not in _VALID_MODES:
                raise SeahorseError(
                    ...
                    detail=(
                        ...
                        "engine.remember directly; 'llm_partial' is reserved)"
                    ),
                )
    ...
    def recall(
        ...
        *,
        ...
        if pit is not None and not getattr(self._retriever, "supports_pit", False):
            ...
            # The hybrid later-release retriever declares ``supports_pit`` and
            # receives the pit verbatim.
    ...
    def recall_timeline(
        ...
        *,
        ...
        """Recall the TIMELINE level (delegates to ``materialize_timeline``).

        ``hops`` is the BFS traversal depth (1-2; >2 raises ``HopsCapExceeded``
        in the shaper). ``pit=None`` resolves to ``state_at`` at the facade
        clock (no silent ``known_at``).
        """
        if pit is not None:
    ...
    def recall_full(
        ...
        """Recall the FULL level (delegates to ``materialize_full``).

        Border shape-validation only: ``pit.kind`` is validated here. The
        ``MAX_FULL_BATCH`` cap and ``PitFullNotSupported`` are the shaper's
        domain contract — it raises ``FullBatchTooLarge`` (before any fetch) and
        ``PitFullNotSupported`` (PIT in FULL is a later release). The facade
        does NOT replicate the batch guard (delegation purity: MAX_FULL_BATCH is
        owned by the shaper).
        """
        if pit is not None:
            self._validate_pit_kind(pit.kind)
    ...
    def improve(
        ...
        *,
        ...
        """Improve a fact (human edit): delegates to ``engine.improve`` directly.

        Manual path (NOT the write path): the write path owns the first-ingestion
        write-path, not the supersede edit. The effective provenance marks the
        edit as a skip-path extraction (``extraction_mode='skip'``,
        ``model_used=None``, ``prompt_hash=None``, ``confidence=1.0`` — a human
        edit is fully trusted) while preserving the caller's ``source_type``
        (caller authority). The facade does NOT open ``repo.atomic()`` (the
        engine owns the atomic invalidate-then-append) and does NOT call
        ``write_path.ingest``.
    ...
    def forget(
        ...
        *,
        ...
        """Forget a fact (soft-delete): delegates to ``engine.forget`` directly.

        ``EngineError(E_PENDING_CANNOT_INVALIDATE)``, ``InvalidationConflictError``,
        and ``NotFound`` are propagated verbatim. The facade does NOT call
    ...
    def get_vigente(self, subject: str | None = None) -> list[Episode]:
        """Currently valid episodes (passthrough to ``get_vigente``).

        Exposed for the consolidate CLI: the distillation clusters the
        current-state set by subject recurrence.
        """
    ...
    def distill(
        ...
        Delegates to the ``distill_episodes`` primitive (a client of the engine)
    ...
    def audit_log(self, ep_id: str) -> list[AuditEvent]:
        """Audit events for ``ep_id`` (delegates to ``engine.audit_log``)."""
        return self._engine.audit_log(ep_id)

    def follow_supersedes_chain(self, ep_id: str) -> list[Episode]:
        """Supersedes closure for ``ep_id`` (delegates to the engine)."""
    ...
    def context(self, *, top_k: int | None = None) -> ContextData:
        ...
        # Deterministic sort: created_at desc, ep_id asc tie-break (two stable
        # sorts, mirroring the current-state retriever).
    ...
    def expire(self, ep_id: str) -> Episode:
        """Decay (set ``expired_at``) — a medium-term goal. Refused in the current release."""
        raise SeahorseError(
            code=E_NOT_IN_MVP_0_1,
            detail="expire is not in the current release (decay is a medium-term goal)",
        )

    def revalidate(self, ep_id: str, *, by: Provenance) -> WriteResult:
        """SUPERSEDED → ACTIVE via a new episode — a later release. Refused in
        the current release."""
        raise SeahorseError(
            code=E_NOT_IN_MVP_0_1,
            detail="revalidate is not in the current release (a later-release primitive)",
        )
