"""SqliteEpisodeRepository — implementation of the engine ``EpisodeRepository`` Protocol.

Implements ``seahorse.contracts.engine.EpisodeRepository`` over the
``ConnectionManager``. There is NO ``delete``, NO ``update_body``,
NO ``update_valid_at`` (retain-not-delete). Each mutator wraps itself in
...
Bi-temporal axes: ``query_state_at`` filters on ``valid_at``/``invalid_at``,
...
# The episode_index INSERT column list is derived from the single source in
# ``episode_index_columns`` (the hot path writes the 15 core columns, leaving
# file metadata NULL). The VALUES tuple below must stay in
...
class SqliteEpisodeRepository:
    """SQLite implementation of the ``EpisodeRepository`` Protocol."""
    ...
    def append(self, episode: Episode) -> None:
        ...
        A second currently-valid row for the same ``fact_id`` raises
        ``IntegrityError`` (mirrors the ``episodes`` partial unique index); the
        Engine must invalidate the predecessor first via ``set_invalid_at``.
        """
    ...
    def set_invalid_at(self, ep_id: str, now: datetime) -> None:
        """Idempotent invalidation: ``invalid_at`` set once.
        ...
        0 rows). Propagates to ``episode_index`` and to the vec0 auxiliary column
        ``vec_episodes.invalid_at`` so the current-state kNN pushdown excludes
        invalidated episodes. The FTS5 index needs no UPDATE (search filters via
        the ``episode_index`` JOIN).
        """
        with self._cm.atomic() as w:
    ...
    def query_state_at(self, t: datetime, subject: str | None = None) -> list[Episode]:
        ...
        # OR invalid_at > t). valid_at IS NULL = "from forever" — valid at ANY t,
        # so it is INCLUDED (mirrors the canonical engine predicates
        # get_vigente / is_valid_at, which already include NULL). Real PENDING is
        # valid_at in the FUTURE, excluded by ``valid_at <= t``.
        where = (
