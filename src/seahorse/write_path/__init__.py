"""Write Path — the single write entry behind the facade, the MCP server, and
the CLI.

``WritePath`` is the Protocol ``MemoryFacade.remember`` delegates to. The first
release ships ``StubWritePath`` (skip path real, llm→skip degrade). The
first-class decision tree (formal ``is_valid_skip_path`` gate +
``deterministic_extract`` fallback + real llm path) is planned for a later
release.
"""
...
class WritePath(Protocol):
    """The single write-path extension point ``MemoryFacade.remember`` delegates to.
    ...
    the first release). Returns the engine ``WriteResult`` verbatim.
    """
