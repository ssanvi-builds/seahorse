"""Model pinning — tag + digest identity (f5-16 §5.2 F3).

``pin_ollama_model`` reuses #4's digest-resolution mechanism (``ollama show``)
to pin a model by its SHA-256 digest — the manifest records
...
def pin_ollama_model(model_tag: str) -> ModelPin:
    ...
    Reuses #4's digest-resolution mechanism (the ollama CLI) — not
    ``#4.complete()``. Raises if the tag is not pulled locally (forces an
    explicit pre-pull, ADR-10).
    """
